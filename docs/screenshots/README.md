...
1. App starten und mit Beispieldaten füllen — keine echten Namen, keine echte
   Firma. Für die Bilder eignen sich `Mustermann, Max` und `Beispiel GmbH`.
2. Fenster auf eine übliche Größe ziehen (etwa 1180 × 780).
...
5. Für die beiden PDF-Bilder dieselbe Woche zweimal ausgeben: einmal unter
   **Einstellungen → PDF-Layout → Klassisch**, einmal unter **Modern**.

Breite von etwa 1200 px reicht völlig; größere Bilder blähen das Repository nur auf.

> Zum Prüfen eines Layouts während der Entwicklung gibt es `npm run uishot` — das
> öffnet die gebaute Oberfläche mit erfundenen Daten und nimmt jede Ansicht auf.
> Die Bilder für die README entstehen weiterhin von Hand, damit echte Inhalte
